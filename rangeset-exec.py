...
def init_arg_parser():
    ...
    parser = argparse.ArgumentParser(description='Execution of CLI commands with clustershell RangeSet support for decimal and hexadecimal numbers')
    ...
    parser.add_argument('-c',
                        ...
                        required=True,
                        type=str,
                        help="Set command to be executed with positional rangeset placeholder: '{0}'")
    ...
    parser.add_argument('-r',
                        ...
                        required=True,
                        type=str,
                        help="Specify decimal or hexadecimal range set e.g. '0-10,27-45,60-100/2' or '0x00ab,0x00cd-0x00ff'")
    ...
    parser.add_argument('-p',
                        ...
                        required=False,
                        action='store_true',
                        help='Enable print mode to show commands generated')
...
def decimal_repl(match):
    "Return the decimal string for a hex number"
...
def main():
    ...
    try:
        ...
        regex_idx_pattern = re.compile(r'\{0\}')

        regex_hex_pattern = re.compile(r'(0[xX][0-9a-fA-F]+)+')
        regex_hex_result = regex_hex_pattern.sub(decimal_repl, args.rangeset)

        rangeset_numbers = []
        rangeset_striter = RangeSet(regex_hex_result).striter()

        if args.use_hex:
            for i in rangeset_striter:
                hex_number = args.format_hex.format(int(i))
                rangeset_numbers.append(hex_number)
        else:
            rangeset_numbers.extend(rangeset_striter)

        for i in rangeset_numbers:
            cmd_list.append(regex_idx_pattern.sub(i, args.cmd, 1))
        ...
        if args.print_mode:
            ...
            for cmd_item in cmd_list:
                subprocess.run(cmd_item.split())

    except Exception:
        logging.exception('Caught exception main')
